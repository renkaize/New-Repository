...
//	while((atk_8266_consta_check()-'2'));   //获得IP
	...
	//atk_8266_send_cmd("AT+ATKCLDSTA=\"30867703608463315810\",\"12345678\"","OK",50);   //0：单连接，1：多连接
PRESTA:
	...
	{
		LCD_ShowString(15,160,200,16,16,"DHT11 Error");
		delay_ms(200);
		LCD_Fill(15,160,239,130+16,WHITE);
		delay_ms(200);
	}
	LCD_ShowString(15,160,200,16,16,"DHT11 OK");
	while(1)
	{
		...
		{									  
			...
			LCD_ShowString(15,260,200,16,16,"Temp:  C");	 
			LCD_ShowString(15,280,200,16,16,"Humi:  %");
			POINT_COLOR=RED;
			LCD_ShowNum(15+40,260,temperature,2,16);		//显示温度	   		   
			LCD_ShowNum(15+40,280,humidity,2,16);			//显示湿度	 		
		}				   
		delay_ms(10);
		a++;
		if(t==20) 
		...
		else if(key==KEY1_PRES)	//KEY1 向ID1发送数据 
		{
			sprintf((char*)p,"温度:%dC,湿度:%d%%\r\n",temperature,humidity);//测试数据
			Show_Str(15+54,125,200,12,p,12,0);
			atk_8266_send_cmd("AT+CIPSEND=1,19","OK",200);  //发送指定长度的数据
			delay_ms(200);
			atk_8266_send_data(p,"OK",100);  //发送指定长度的数据
			timex=100;
		}
