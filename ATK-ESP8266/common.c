...
const u8* wifista_ssid="FAE";			//路由器SSID号
...
//	u16 rlen=0;
	...
	u8 humidity=1; 
	POINT_COLOR=RED;
	Show_Str(30,100,200,16,"发送数据:",16,0); 	//发送数据
	Show_Str(30,130,200,16,"接收数据:",16,0);	//接收数据
	Show_Str_Mid(0,30,"ATK-ESP8266 WIFI模块测试",16,240);
	
	atk_8266_send_cmd("AT+ATKCLDCLS","OK",200);
	atk_8266_send_cmd("AT+RST","OK",200);
	delay_ms(2000);
	key = atk_8266_send_cmd("AT","OK",20);
	while(key==0)//检查WIFI模块是否在线
	{
		
		key=atk_8266_send_cmd("AT+CWMODE=1","OK",20);
		printf("开始连接原子云请等待\r\n");
		if(key==0)
		{
			printf("AT+CWMODE=1指令成功\r\n");
			key =atk_8266_send_cmd("AT+CWJAP=\"FAE\",\"15902020353\"","OK",2000);//自己可用的路由账号及密码
			while(key !=0)
			{
				tim++;
				key =atk_8266_send_cmd("AT+CWJAP=\"FAE\",\"15902020353\"","OK",2000);
				if(tim%30==0)
				{
				printf("连接WIFI失败，请检查下WIFI账号及密码是否设置准确\r\n");
				}
			}
		}
		else printf("AT+CWMODE=1指令失败\r\n");
		if(key == 0)
		key = atk_8266_send_cmd("AT+ATKCLDSTA=\"89777454978378069999\",\"12345678\"","OK",200);//原子云的设备号  及密码！！！
		if(key==0)
		{
		printf("连接原子云成功,按KEY0可发送温湿度值到原子云上\r\n");
		USART3_RX_STA=0;/*清空串口三的状态标志位，由于前面的AT指令会使8266接收到指令后返回数据进入串口三的中断函数，
						导致标志位有会增加却没清空，会影响下边的串口三接收函数的判断所以在这里先清空*/
		}
		else  printf("连接原子云失败,请检查原子云设备号及密码是否准确，若确认无误请重新刷写原子云固件\r\n");
		break;
	} 
		while(DHT11_Init())	//DHT11初始化	
			{
				LCD_ShowString(30,160,200,16,16,"DHT11 Error");
				delay_ms(200);
				LCD_Fill(30,160,300,130+16,WHITE);
				delay_ms(200);
			}
			LCD_ShowString(30,160,200,16,16,"DHT11 OK");
		while(1)
		{
			if(a%10==0)//每100ms读取一次
				{									  
					...
					LCD_ShowString(30,250,200,16,16,"Temp:  C");	 
					LCD_ShowString(30,270,200,16,16,"Humi:  %");
					POINT_COLOR=RED;
					LCD_ShowNum(30+40,250,temperature,2,16);		//显示温度	   		   
					LCD_ShowNum(30+40,270,humidity,2,16);			//显示湿度	 		
				}				   
				delay_ms(10);
				a++;
				key=KEY_Scan(0);
				if(key==KEY0_PRES)			//WK_UP 退出测试		 
				{  
					LCD_Fill(30+80,100,239,112,WHITE); //清屏接收区域的接收缓存
					
					sprintf((char*)p,"温度:%d C,湿度:%d %% \r\n",temperature,humidity);//测试数据
					Show_Str(30+80,100,200,16,p,16,0);
					u3_printf("%s",p);												 
				}
		//if(timex)timex--;
		//if(timex==1)LCD_Fill(30+54,100,239,112,WHITE);
		if(USART3_RX_STA&0X8000)			//接收到一次数据了，前面清除操作就是在这里用到了
		{
			LCD_Fill(30+80,130,479,145,WHITE); //清屏接收区域的接收缓存
 			reclen=USART3_RX_STA&0X7FFF;	//得到数据长度
		  	USART3_RX_BUF[reclen]=0;	 	//加入结束符  
			//LCD_ShowString(30+80,130,200,16,16,USART3_RX_BUF);
			Show_Str(30+80,130,156,16,USART3_RX_BUF,16,0); 			//显示接收到的数据
			for(a=0;a<reclen;a++)
			{	
				while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);//等待发送结束
				USART_SendData(USART1, USART3_RX_BUF[a]);         //向串口1发送数据
			}
			
			printf("\r\n\r\n");//插入换行
			USART3_RX_STA=0;
			memset(USART3_RX_BUF,0,sizeof(USART3_RX_BUF));	//清空USART3_RX_BUF的数据		
		}	 
...
//				USART_SendData(USART3, USART_RX_BUF[a]);         //向串口2发送数据
//			}
//			printf("\r\n\r\n");//插入换行
