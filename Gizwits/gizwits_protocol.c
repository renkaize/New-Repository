...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueACCA_X != cur->valueACCA_X)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueACCA_X Changed\n");
            ret = 1;
        }
    }
    if(last->valueACCA_Y != cur->valueACCA_Y)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueACCA_Y Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueACCA_X = gizY2XFloat(ACCA_X_RATIO,  ACCA_X_ADDITION, dataPoints->valueACCA_X); 
    devStatusPtr->valueACCA_Y = gizY2XFloat(ACCA_Y_RATIO,  ACCA_Y_ADDITION, dataPoints->valueACCA_Y); 
